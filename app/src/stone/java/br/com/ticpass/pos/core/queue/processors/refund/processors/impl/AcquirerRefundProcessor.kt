 ...
 * Processes refunds using the Stone SDK via direct SDK access.
 */
class AcquirerRefundProcessor @Inject constructor() : RefundProcessorBase() {
    ...
    override suspend fun processRefund(item: RefundQueueItem): ProcessingResult {
        try {
            ...
            val refundProviderFactory = AcquirerSdk.refund.getInstance()
            val (provider, transaction) = refundProviderFactory(item.atk)
