...
import br.com.ticpass.pos.core.nfc.models.NFCTagCartHeader
 ...
 */
@Singleton
class NFCCartStorage @Inject constructor(
    ...
    suspend fun readCartHeader(sectorKeys: NFCTagSectorKeys): NFCTagCartHeader? {
        return try {
            val headerBlock = nfcOperations.readBlock(0, 2, sectorKeys)
            if (headerBlock == null) {
    ...
    suspend fun writeCartHeader(header: NFCTagCartHeader, sectorKeys: NFCTagSectorKeys) {
        try {
            ...
            val success = nfcOperations.writeBlock(0, 2, headerBytes, sectorKeys)
    ...
    suspend fun readCart(header: NFCTagCartHeader, sectorKeys: NFCTagSectorKeys): List<NFCCartItem> {
        try {
            ...
            while (true) {
                val blockData = nfcOperations.readBlock(currentSector, currentBlock, sectorKeys)
                if (blockData == null) {
    ...
    suspend fun writeCart(
        ...
        try {
            ...
            while (dataOffset < totalBytes) {
                ...
                // Write block
                val success = nfcOperations.writeBlock(currentSector, currentBlock, blockData, sectorKeys)
                if (!success) {
    ...
    suspend fun readCustomerHeader(sectorKeys: NFCTagSectorKeys): NFCTagDataHeader? {
        return try {
            val headerBlock = nfcOperations.readBlock(0, 1, sectorKeys)
            if (headerBlock == null) {
