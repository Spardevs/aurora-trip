...
import br.com.ticpass.pos.queue.processors.printing.utils.PrintQueueFactory
        ...
        viewModelScope.launch {
            printEvents.collect { event ->
                when (event) {
                    ...
                    is PrintingEvent.ConnectingToPrinter -> _printerStatus.value =
                        PrinterStatus.Connecting
                    is PrintingEvent.PrinterConnected -> _printerStatus.value =
                        PrinterStatus.Connected(event.printerName)
                    is PrintingEvent.OutOfPaper -> _printerStatus.value = PrinterStatus.OutOfPaper
                    is PrintingEvent.Progress -> _printerStatus.value =
                        PrinterStatus.Printing(event.percentComplete)
                    is PrintingEvent.Completed -> _printerStatus.value = PrinterStatus.Idle
                    is PrintingEvent.Failed -> _printerStatus.value =
                        PrinterStatus.Error(event.error)
