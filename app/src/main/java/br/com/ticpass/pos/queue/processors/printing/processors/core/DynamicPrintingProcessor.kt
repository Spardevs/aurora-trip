...
/**
 * Dynamic Printing Processor
 * A processor that delegates to different concrete processors based on the printing item's processor type
 * Allows multiple printing types to be processed in a single queue
 */
class DynamicPrintingProcessor(
    ...
    private var currentDelegateProcessor: PrintingProcessorBase? = null
    ...
    override suspend fun processPrinting(item: PrintingQueueItem): ProcessingResult {
        ...
        val processor = processorMap[processorType] ?:
                        processorMap[PrintingProcessorType.ACQUIRER] ?: // Fallback to acquirer
                        return PrintingError(ProcessingErrorEvent.PROCESSOR_NOT_FOUND)
        ...
        try {
            // We can't directly call processPrinting since it's protected
    ...
    override suspend fun onAbort(item: PrintingQueueItem?): Boolean {
        ...
        val processorType = item?.processorType ?: PrintingProcessorType.ACQUIRER
        val processor = processorMap[processorType] ?:
                        processorMap[PrintingProcessorType.ACQUIRER] ?: // Fallback to acquirer
                        return false
     ...
     */
    private fun launchEventForwarding(sourceEvents: SharedFlow<PrintingEvent>): Job {
        ...
        return scope.launch {
            sourceEvents.collect { event ->
                ...
                if (event !is PrintingEvent.START) {
                    _events.emit(event)
                }
            }
        }
